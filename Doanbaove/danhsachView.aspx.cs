...
namespace Doanbaove
{
    public partial class danhsachView : System.Web.UI.Page
    {
        ...
        protected void btt_add_Click(object sender, EventArgs e)
        {
            try
            {
                string st_msv, st_sql,st_mdk;
                st_mdk = tbx_del.Text.ToUpper();
                ...
                else
                {
                    ...
                    if (check > 0 && check1 > 0)
                    {
                        Response.Redirect("danhsachView.aspx?menu=mh&type=mh&MaMH="+ st_mamh);
        ...
        //    string st_mdk;
        //    string st_sql = "";
        //    st_mdk = tbx_search.Text.ToUpper();
        //    st_sql = "Select madk as 'Mã điều kiện',masv as 'Mã sinh viên', tensv as 'Tên sinh viên',Case Gioitinh when 1 then N'Nữ' else N'Nam' end as 'Giới tính', Khoa as 'Khóa' from tbl_addmonhoc inner join tbl_sinhvien on masv=masv";
        //    st_sql = st_sql + "  WHERE masv Like '%" + st_mdk + "%'  OR UPPER(tensv) LIKE N'%" + st_mdk + "%' OR UPPER(madk) LIKE N'%" + st_mdk + "%' ORDER BY madk;";
        //}
        ...
        protected void btt_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string st_mamh = Request.QueryString.Get("MaMH").ToString();
                cls_con.connect_Data();
                st_sql = "SELECT count(masv) FROM tbl_addmonhoc WHERE masv='" + st_msv + "'and mamh=" + st_mamh;
                sqlcm = new SqlCommand(st_sql, cls_con.con);
                int k = Convert.ToInt16(sqlcm.ExecuteScalar());
                if (k == 0)
                {
                    lbl_tb.Text = "Lỗi: Sinh viên không học môn học này!";
                    lbl_tb.Visible = true;
                ...
                else
                {
                    ...
                    int check;
                    st_sql = "delete from tbl_addmonhoc where masv='" + st_msv + "'and mamh=" + st_mamh;
                    sqlcm = new SqlCommand(st_sql, cls_con.con);
                    check = sqlcm.ExecuteNonQuery();
                    if (check == 1)
                    {
        ...
        protected void btt_del1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (check == 1)
                    {
                        Response.Redirect("danhsachView.aspx?menu=lop&type=lop&Malop=" + st_malop);
                        lbl_tb.Text = "Xóa sinh viên thành công!";
                        lbl_tb.Visible = true;
                    }
                    else
